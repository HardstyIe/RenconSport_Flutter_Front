  ...
  void addFilter(BuildContext context) {
    showDialog(
      ...
      builder: (BuildContext context) {
        Sport selectedSport =
            sports[0]; // Sélectionnez le premier sport par défaut.
              ...
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    DropdownButtonFormField<Sport>(
                      ...
                      items: sports.map((sport) {
                        return DropdownMenuItem<Sport>(
                          value: sport,
                          child:
                              Text(sport.name, style: TextStyle(fontSize: 18)),
                        );
                      }).toList(),
                      onChanged: (sport) {
                        setState(() {
                    ...
                    TextFormField(
                      decoration:
                          InputDecoration(labelText: 'Nombre de Joueurs'),
        ...
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final startDate = DateTime(filter.date.year, filter.date.month,
                  filter.date.day, filter.time.hour, filter.time.minute);
              final endDate = startDate.add(Duration(minutes: filter.timeOut));

              return ListTile(
                title:
                    Text('Filter ${index + 1}', style: TextStyle(fontSize: 18)),
                subtitle: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Sport: ${filter.sport.name}',
                        style: TextStyle(fontSize: 18)),
                    Text('Heure de début: ${startDate.toString()}',
                        style: TextStyle(fontSize: 18)),
                    Text('Heure de fin: ${endDate.toString()}',
                        style: TextStyle(fontSize: 18)),
                    ElevatedButton(
                      onPressed: () async {
                        // Récupérez les détails du sport.
                        final sportDetails =
                            await fetchSportDetails(filter.sport.name);
                          ...
                          showDialog(
                            ...
                            builder: (BuildContext context) {
                              return AlertDialog(
                                title: Text('Erreur'),
                                content: Text(
                                    'Impossible de récupérer les détails du sport.',
                                    style: TextStyle(fontSize: 18)),
                                actions: [
                                  TextButton(
                                    onPressed: () {
...
class SearchBarApp extends StatefulWidget {
  ...
  SearchBarApp({required this.sports, required this.searchResults, Key? key})
      : super(key: key);
                ...
                onChanged: (query) {
                  setState(() {
                    widget.searchResults
                        .clear(); // Effacez la liste des résultats de recherche actuelle.
                    updateSearchResults(
                        query); // Mettez à jour la liste des résultats de recherche.
                    controller.openView();
                  });
                },
